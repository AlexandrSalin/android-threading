 ...
 * class in the cache.
 *
 * @param <T> the class of the objects this {@link com.octo.android.robospice.persistence.ObjectPersister} can
 *            persist/unpersist.
 * @author sni
 */
public abstract class ObjectPersister<T> implements Persister, CacheCleaner {
   ...
   * @param maxTimeInCache the maximum time the data can have been stored in cached
   *                       before being considered expired. 0 means infinite.
   * @return the data if it could be loaded.
   * @throws com.octo.android.robospice.persistence.exception.CacheLoadingException if the data in cache is expired.
   */
   ...
   * cacheKey.
   *
   * @param cacheKey the cachekey identifying the object to look for.
   * @return a long corresponding to the creation date of creation of cache.
   * @throws com.octo.android.robospice.persistence.exception.CacheLoadingException if there is no such element in cache.
   */
